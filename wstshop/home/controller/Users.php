...
use wstshop\common\model\Users as MUsers;
use wstshop\common\model\LogSms;
/**
 * ============================================================================
 * WSTShop网上商店
...
class Users extends Base{
    ...
    public function getPhoneVerifyCode(){
        ...
        $msg = "欢迎您注册成为".WSTConf("CONF.shopName")."会员，您的注册验证码为:".$phoneVerify."，请在10分钟内输入。【".WSTConf("CONF.shopName")."】";
        $m = new LogSms();
    ...
    public function index(){
        ...
        // 我的关注
        $favorites = model('Favorites')->listGoodsQuery();
        $this->assign('favorites',$favorites['Rows']);
        // 获取用户各订单状态数
        $order = model('Orders')->getOrderStatusNum();
    ...
    */
    ...
    public function getEmailVerify(){
        ...
        您在".date('Y-m-d H:i:s')."发出了绑定邮箱的请求,本次进行绑定邮箱的验证码为:$code<br>
    ...
    */
    ...
    public function getEmailVerifyt(){
        ...
        $code = rand(0,999999);
        $html="您好，会员 ".session('WST_USER.loginName')."：<br>
        您在".date('Y-m-d H:i:s')."发出了修改邮箱的请求,本次进行修改邮箱的验证码为:$code<br>
        该验证邮件有效期为30分钟，超时请重新发送邮件。<br>
        <br><br>*此邮件为系统自动发出的，请勿直接回复。";
        $sendRs = WSTSendMail($userEmail,'绑定邮箱',$html);
        if($sendRs['status']==1){
    ...
    */
    ...
    public function getPhoneVerifyo(){
        ...
        $phoneVerify = rand(100000,999999);
        $msg = "欢迎您".WSTConf("CONF.shopName")."会员，正在操作绑定手机，您的校验码为:".$phoneVerify."，请在10分钟内输入。【".WSTConf("CONF.shopName")."】";
        $m = new LogSms();
    ...
    public function getPhoneVerifyt(){
        ...
        $phoneVerify = rand(100000,999999);
        $msg = "欢迎您".WSTConf("CONF.shopName")."会员，正在操作修改手机，您的校验码为:".$phoneVerify."，请在10分钟内输入。【".WSTConf("CONF.shopName")."】";
        $m = new LogSms();
        $rv = $m->sendSMS(0,$userPhone,$msg,'getPhoneVerify',$phoneVerify);
        if($rv['status']==1){
            $USER = '';
    ...
    */
    public function editUserPhoto(){
        ...
        if($rs){
            // 直接保存裁剪后的头像进数据库
            $rs1 = model('users')->saveUserPhoto($imageSrc);
            if($rs1==false)return WSTReturn('图像保存失败',-1);
    ...
    public function getfindPhone(){
        ...
        $phoneVerify = rand(100000,999999);
        $msg = "您正在重置登录密码，验证码为:".$phoneVerify."，请在10分钟内输入。【".WSTConf("CONF.shopName")."】";
        $m = new LogSms();
        session('WST_USER',null);
